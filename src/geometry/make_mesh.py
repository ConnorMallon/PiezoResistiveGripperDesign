...
lc_extra_ref = 0.0005 * factor # extra refinement around specific points
...
# 6) Top boundary splits at x = [0, .12, .125, .14, Lx]
...
# 11) Interior feature-lines e3…e6 (NOW at y_elec = 0.045)
y_elec = 0.0045
interior = [
    ((0.02, y_elec), (0.025, y_elec)),  # e3
    ((0.06, y_elec), (0.065, y_elec)),  # e4
    ((0.00, y_elec), (0.005, y_elec)),  # e5
    ((0.04, y_elec), (0.045, y_elec)),  # e6
]
e_lines = []
...
for dim, tag in line_entities:
    ...
    # interior e-lines (NOW at y_elec)
    elif abs(y - y_elec) < tol:
...
# 2) D1 gaps along y = y_elec  (moved from y_D1)
yD1    = y_elec
e5_rng = (0.00, 0.005)
e3_rng = (0.02, 0.025)
e6_rng = (0.04, 0.045)
...
# 3) D2 gaps along y = 0.065 (unchanged)
yD2    = 0.065
e2_rng = (0.12,  0.125)
f1_rng = (0.14, Lx)
...
# ——————————————————————————————————————————————————————————————————
# 5) Vertical D1/D2 interior borders (unchanged)
D1_vert, D2_vert = [], []
xD1b = 0.085
xD2b = 0.12

for tag, (x, y) in lines.items():
    if abs(x - xD1b) < tol and 0.0 - tol <= y <= y_D1 + tol:
        D1_vert.append(tag)
...
# ————————————————————————————————————————————————————————————————
# Physical groups for the vertical sides of D1 (bottom band) and D2 (top band) — UNCHANGED
tol = 1e-6
Lx, Ly = 0.15, 0.07
y_D1 = 0.008
y_D2 = 0.065
...
# 16) Mesh-size fields for smooth refinement (unchanged)
...
# Balls near (0.14,0.07) and (0.075,0.0)
...
gmsh.model.mesh.field.setNumber(f_ball1, "Radius",  0.005)
...
gmsh.model.mesh.field.setNumbers(f_min, "FieldsList",
    [ f_thresh, f_box, f_ball1, f_ball2 ]
)
gmsh.model.mesh.field.setAsBackgroundMesh(f_min)

# 17) Generate and write
gmsh.model.mesh.generate(2)
gmsh.write("domain_with_e.msh")
print("Wrote domain_with_e.msh")
